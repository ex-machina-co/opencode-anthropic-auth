...
Anthropic gates model access on the reported Claude Code version server-side, returning a 400 `claude_code_version_too_old` error for models that require a newer client. On OpenCode v2, when the exact structured rejection names a newer minimum, the plugin adopts that real version for the rest of the process and retries the initial request once. Other 400 responses, malformed version errors, and explicit `ANTHROPIC_CLAUDE_CODE_VERSION` overrides are never changed automatically. The override remains an escape hatch for both release lines.
